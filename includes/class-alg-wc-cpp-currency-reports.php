...
class Alg_WC_CPP_Currency_Reports {
	 ...
	 * @since    1.1.0
	 */
	function get_node_meta_title( $code ) {
		return ( 'merge' === $code ?
	...
	function add_reports_currency_to_admin_bar( $wp_admin_bar ) {
		if ( isset( $_GET['page'] ) && 'wc-reports' === $_GET['page'] ) {
			...
			$_current_code = isset( $_GET['currency'] ) ? $_GET['currency'] : $base_currency;
			...
			if ( '' != ( $wc_currency = get_woocommerce_currency() ) ) {
	...
	/**
	 * change_reports_currency_code.
	 *
	 * @version  1.1.0
	 * @since    1.1.0
	 */
	function change_reports_currency_code( $currency ) {
		if ( isset( $_GET['page'] ) && 'wc-reports' === $_GET['page'] && isset( $_GET['currency'] ) ) {
